...
if (isset($_POST["login"])){
    ...
    if(sanea($_POST['clave'], 'string', 8, 45, '/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$/')) {
        ...
        $estado .= ' Error - La contrase&ntilde;a debe tener al menos 8 caracteres, y contener mayúsculas, min&uacute;sculas y n&uacute;meros. ';
    ...
    if($estado == '' && $usuario = modelUsuarios::comprobarCredenciales($nick, $clave)) {
        ...
        if($usuario['activo'] == 1) {
            ...
            $estado .= ' Error - El usuario '.$usuario['nick'].' no está activo. ';
            $estado .= ' Contacte con el administrador para este . ';
        }
            
    }else{
        $_SESSION['usuario'] = NULL;
        $estado .= ' Error - Contraseña o usuario incorrectos.';
